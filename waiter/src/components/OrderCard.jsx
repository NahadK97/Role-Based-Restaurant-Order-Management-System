  ...
  const handleButtonClick = async () => {
    ...
    if(status === "ready") {
      ...
      try {
        ...
        const response = await axios.patch(`http://localhost:4000/api/${RID}/orders/edit/${order.tableNo}`,newOrder); 
        setStatus("locked");
      }
      catch(err) {
      ...
      try{
        ...
        const total = await CalcTotal(orders, menu.flatMap(category => category.dishes));
        ...
        if(checkResponse.data) {
          ...
          const patchResponse = await axios.patch(`http://localhost:4000/api/${RID}/tables/${order.tableNo}/edit`,edits)
        }
        else {
          const newTable = {
            RID : RID,
        ...
        const deleteResponse = await axios.delete(`http://localhost:4000/api/${RID}/orders/delete/${order.tableNo}`)
      }
      catch(err) {
